# Retrieval benchmarks — 2026-05-07 (rerun after V2 ingest / RAG work)

Snapshot of **Recall@10** runs from the `slow-tests` harness. Numbers below are copied from **`cargo test` output with `--nocapture`** so `println!` metrics are visible.
...
|-------|--------|
| **Date (UTC)** | 2026-05-07 |
| **Git revision** | `bbd2b97` |
| **Crate** | `src-tauri` (`curated-thoughts` / `tauri_app_lib`) |
| **Features** | `test-utils`, `slow-tests` |
| **SciFact wall time (this run)** | ~186 s for `tests/scifact.rs` (2 tests, printed by harness) |
...
- Eval denominator in logs: **300** queries (`Recall@10` lines use `hits/300`).
...
Both tests printed a short list of missed claims (first 10); raw log excerpt:

**fulltext-single** missed 53 queries total; printed: claims 198, 1226, 660, 716, 870, 312, 554, 1049, 1175, 623 (truncated preview text in log).

**sentence-chunk** missed 57 queries total; printed: claims 132, 437, 775, 887, 544, 716, 517, 1049, 502, 659 (truncated preview text in log).
...
## Comparison to prior snapshot (same day, earlier commit)

| Metric | [2026-05-07-recall-benchmarks.md](./2026-05-07-recall-benchmarks.md) (`b0ef54c`) | This run (`bbd2b97`) |
|--------|--------------------------------------------------------------------------------|----------------------|
| SciFact fulltext-single | 0.823 (247/300) | **0.823** (247/300) |
| SciFact sentence-chunk | 0.810 (243/300) | **0.810** (243/300) |
| code-curated | 1.000 | **1.000** |
| code-synthetic | 1.000 | **1.000** |
| yaml-k8s-curated | 1.000 | **1.000** |
| yaml-synthetic | 0.958 (69/72) | **0.958** (69/72) |

All headline Recall@10 figures match the earlier snapshot bit-for-bit on this machine.
...
SciFact only (single-threaded, quieter logs):
