...
/// sha256 of `bytes` as a lowercase hex string. Avoids relying on the
/// `LowerHex` impl on `GenericArray` (which depends on `generic_array`
/// features that `sha2` doesn't enable by default in 0.10+).
...
///
/// Path normalization (4 stages, per spec section 4):
...
pub fn enqueue_vault_event(
    ...
    if matches!(event_kind, notify::EventKind::Remove(_)) {
        conn.execute(
...
mod tests {
    ...
    /// coupling `tools` to the canonical `src-tauri/src/db/schema.rs`
    ...
    /// connection avoids coupling the test to the canonical
    /// `tauri_app_lib` migration stack; if the canonical schema changes
    /// the production upsert paths still validate.
    ...
    fn setup_vault_with_file(body: &[u8]) -> (TempDir, PathBuf) {
    ...
    fn enqueue_modify_indexed_with_same_hash_is_noop() {
        ...
        let canonical = std::fs::canonicalize(&file_path).unwrap();
        let path_str = canonical.to_string_lossy().into_owned();
        let stable_hash = sha256_hex(b"stable");
    ...
    fn enqueue_out_of_vault_path_is_rejected() {
        // Use TempEnv to set CURATED_VAULT_ROOT to a directory that does
        // NOT contain the file we touch — the event must be rejected.
        let inner_vault = TempDir::new().unwrap();
        let outer_vault = TempDir::new().unwrap();
