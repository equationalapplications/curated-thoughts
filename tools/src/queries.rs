//! tools/src/queries.rs
//!
//! Read-only `ct` subcommands: search, recall, code, graph, wiki list, wiki get.
//!
//! All command fns are pure: open a read-only connection, run a query, print
//! human or `--json` output, and return an exit code. No DB writes.
//!
//! `Task 4` of the phase-2 headless CLI plan moved these from `cli_common.rs`
//! verbatim. Helpers are `pub` (not `pub(crate)` or private) because `Task 5`
//! (`cmds.rs`) and the existing `curated_thoughts_mcp` sidecar consume them.
//! The duplicates that still live in `cli_common.rs` will be deleted by
...
use tauri_app_lib::retrieval::{self};
...
// ---------------------------------------------------------------------------
// Search / recall / code — shared plumbing for the three semantic-query
// subcommands.
// ---------------------------------------------------------------------------
...
///
/// Returns `Some((conn, chunks))` when there is at least one ranked chunk to
/// render; `None` for both the empty-query short-circuit and the no-results
/// case. The connection is returned so the wiki ranking leg in `recall_cmd`
/// can reuse it instead of reopening the database.
fn run_query(
    query: &str,
    k: usize,
    ast_only: bool,
...
// ---------------------------------------------------------------------------
// Tests — moved verbatim from cli_common's pre-extracted region (these only
// exercise the local helpers in this module; integration tests live in
// tools/tests/).
// ---------------------------------------------------------------------------
...
// without a seeded brain: `clamp_limit`, `coerce_updated_at`, the constant
// and enum shape, and `first_line` / `wiki_text_line` formatting. Higher
// coverage runs through the integration tests via the compiled `ct` binary.
#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn clamp_limit_clamps_to_one_to_fifty() {
    ...
    #[test]
    fn coerce_updated_at_handles_int_text_null() {
        assert_eq!(
            coerce_updated_at(Some(rusqlite::types::Value::Integer(42))),
            42
        );
        assert_eq!(
            coerce_updated_at(Some(rusqlite::types::Value::Text("7".into()))),
            7
        );
        assert_eq!(
            coerce_updated_at(Some(rusqlite::types::Value::Text("garbage".into()))),
            0
        );
        assert_eq!(coerce_updated_at(None), 0);
    }
    ...
    #[test]
    fn graph_max_neighbors_is_200() {
        assert_eq!(GRAPH_MAX_NEIGHBORS, 200);
    }

    #[test]
    fn recall_chunks_sql_base_and_ast_filter_are_distinct() {
        // Sanity: the SQL base + AST filter must concatenate cleanly into a
        // valid WHERE clause without overlap. The exact strings are part of
        // the contract with the MCP sidecar.
        assert!(RECALL_CHUNKS_SQL_BASE.contains("WHERE d.status = 'indexed'"));
        assert!(RECALL_CHUNKS_AST_FILTER.starts_with(" AND "));
        assert!(!RECALL_CHUNKS_AST_FILTER.contains("indexed"));
        let joined = format!("{RECALL_CHUNKS_SQL_BASE}{RECALL_CHUNKS_AST_FILTER}");
        assert!(joined.matches("WHERE").count() == 1);
    }

    #[test]
    fn wiki_text_line_renders_id_and_title() {
        let v = serde_json::json!({"id": "wiki-1", "title": "Helper facts"});
        assert_eq!(wiki_text_line(&v), "wiki-1  Helper facts");
        // Missing fields render as empty strings, not panics.
        let v = serde_json::json!({});
        assert_eq!(wiki_text_line(&v), "  ");
    }

    #[test]
    fn graph_dir_value_enum_variants_cover_callees_callers_both() {
        // Just confirm the variants exist; clap derive macros convert them
        // to/from strings at the CLI boundary.
        let _ = GraphDir::Callees;
        let _ = GraphDir::Callers;
        let _ = GraphDir::Both;
    }
