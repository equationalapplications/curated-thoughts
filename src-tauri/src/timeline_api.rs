//! Tauri commands for the Timeline and Tasks feeds (Phase 5).

use crate::db::events::{list_events, TimelineEvent, TimelineFilter};
...
pub fn list_events_cmd(
    ...
    list_events(&guard.0, &filter.unwrap_or_default()).map_err(|e| e.to_string())
}
...
#[tauri::command]
pub fn archive_task_cmd(task_id: String, db_state: State<DbState>) -> Result<(), String> {
    let mut guard = db_state.0.lock().map_err(|e| e.to_string())?;
