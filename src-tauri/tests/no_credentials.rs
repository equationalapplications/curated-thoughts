...
fn onboard_never_writes_api_key_to_config() {
    ...
    temp_env::with_var(
        "CURATED_BRAIN_DIR",
        Some(config_path.parent().unwrap().to_string_lossy().as_ref()),
        || {
            temp_env::with_var(
...
fn doctor_never_echoes_api_key() {
    ...
    with_var("GENERATION_API_KEY", Some("super-secret-key-99999"), || {
        with_var(
            // Also redirect the brain dir: without it the resolved db_path is
            // the LIVE ~/.brain/brain.db (issue #178) -- doctor reads it.
            "CURATED_BRAIN_DIR",
            Some(config_path.parent().unwrap().to_string_lossy().as_ref()),
            || {
                with_var(
